...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  let body: { km_atual?: number; valor_pago?: number; preco_por_litro?: number | null }
  try {
    body = await req.json()
  } catch {
  ...
  const { km_atual, valor_pago, preco_por_litro } = body
  ...
  try {
    const { rows, rowCount } = await pool.query(
      `UPDATE abastecimentos
         SET km_atual = $1, valor_pago = $2, preco_por_litro = $3, litros = $4
       WHERE id = $5
       RETURNING *`,
      [km_atual, valor_pago, preco_por_litro ?? null, litros, id]
    )
    if (!rowCount) return NextResponse.json({ error: 'Registro não encontrado' }, { status: 404 })
